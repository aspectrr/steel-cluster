...
type ReplaceKeyPrefix<Obj, OldPrefix extends string, NewPrefix extends string> = {
  [K in keyof Obj as K extends `${OldPrefix}${infer Rest}` ? `${NewPrefix}${Rest}` : never]: Obj[K];
};
