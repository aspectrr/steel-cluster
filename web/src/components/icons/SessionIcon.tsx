export const SessionIcon = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
      <g clipPath="url(#clip0_5055_44)">
        <rect x="4" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect x="8" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect y="4" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect x="4" y="12" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect x="12" y="4" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect y="8" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect x="8" y="12" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
        <rect x="12" y="8" width="4" height="4" fill="#F0DBFF" fillOpacity="0.979" />
      </g>
      <defs>
        <clipPath id="clip0_5055_44">
          <rect width="16" height="16" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};
