...
export function useSessionBuffer(sessionId: string, duration?: number) {
  ...
  if (
    ...
    paginatedQuery.data?.pages?.[0]?.data?.length
  ) {
    // Inject a mouse move event as the last event to extend the duration to actual session duration
    const firstEvent = paginatedQuery.data.pages[0].data?.[0] as
      | EventWithTime
      | undefined;
    const endEvent: EventWithTime[] | undefined =
      duration && firstEvent
        ? [
            {
              type: 3,
    ...
    initialEventsRef.current = [
      ...(paginatedQuery.data.pages[0].data as EventWithTime[]),
      ...endEvent,
    ];
  }
