...
// GetSessionEventsResponse not available in standalone mode
type GetSessionEventsResponse = any[];
...
export function SessionViewer({ id }: SessionViewerProps) {
	...
	const fetcherRef = useRef<FetcherRef | undefined>({
		fetchNextPage: fetchNextPage as unknown as FetcherRef["fetchNextPage"],
		hasNextPage,
		isFetchingNextPage,
	});
	useEffect(() => {
	...
	const handleBufferEnd = useCallback(async (): Promise<
		eventWithTime[] | undefined
	> => {
	...
	return (
		...
		>
			...
			{!isEventsLoading && !hasEvents && !isLive && (
				<EmptyState expired={isExpired} />
			)}
		</div>
	);
}
